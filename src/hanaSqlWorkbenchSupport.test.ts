...
describe('buildHanaSqlResultHtml batch view', () => {
  ...
  test('pins the horizontal scrollbar to the bottom of each batch section via a fixed table-wrap height', () => {
    const html = buildHanaSqlResultHtml({
      appName: 'finance-uat-api',
      sql: 'SELECT 1 FROM DUMMY; SELECT 2 FROM DUMMY',
      executedAt: '2026-04-25T00:00:00Z',
    ...
    expect(html).toContain('.result-statement-body .result-table-wrap { height: 360px;');
    expect(html).not.toContain('.result-statement-body .result-table-wrap { max-height: 360px;');
  });
