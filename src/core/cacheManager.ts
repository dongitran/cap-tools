...
export class CacheManager {
  ...
  getOrgs(regionId: string, ttlMs = 4 * 60 * 60 * 1000): CfOrg[] | undefined {
    const entry = this.cache.regions[regionId]?.orgs;
    if (!entry || isExpired(entry, ttlMs)) return undefined;
    return entry.data;
  }

  setOrgs(regionId: string, orgs: CfOrg[]): void {
    this.ensureRegion(regionId);
    this.cache.regions[regionId].orgs = { data: orgs, fetchedAt: nowMs() };
    this.persist();
  }
  ...
  setSpaces(regionId: string, orgName: string, spaces: CfSpace[]): void {
    ...
    this.cache.regions[regionId].orgData[orgName].spaces = { data: spaces, fetchedAt: nowMs() };
    this.persist();
  }
  ...
  setApps(regionId: string, orgName: string, apps: CfApp[], spaceName?: string): void {
    this.ensureOrg(regionId, orgName);
    const key = spaceName ?? '__org__';
    this.cache.regions[regionId].orgData[orgName].apps[key] = { data: apps, fetchedAt: nowMs() };
    this.persist();
  }
  ...
  getStats(): { regions: number; orgs: number; apps: number } {
    ...
    for (const region of Object.values(this.cache.regions)) {
      ...
      for (const org of Object.values(region.orgData ?? {})) {
        apps += Object.values(org.apps ?? {}).reduce((sum, entry) => sum + entry.data.length, 0);
      }
    }
  ...
  private ensureOrg(regionId: string, orgName: string): void {
    this.ensureRegion(regionId);
    if (!this.cache.regions[regionId].orgData[orgName]) {
      this.cache.regions[regionId].orgData[orgName] = {
