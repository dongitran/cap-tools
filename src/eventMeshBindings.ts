 ...
 * Cloud Foundry app's VCAP_SERVICES (the synthesized default-env.json produced by
 * `fetchDefaultEnvJsonFromTarget`). Pure and side-effect free so it is unit-testable.
 *
 * An enterprise-messaging binding exposes several messaging protocols plus a
 * management endpoint. The event listener needs three of them:
 *  - `management[0]`                 -> Event Mesh Management REST API (create/list queues + subscriptions)
 *  - `messaging[protocol "httprest"]`-> REST messaging endpoint (kept for completeness)
 *  - `messaging[protocol "amqp10ws"]`-> AMQP 1.0 over WebSocket (the listen transport)
 * Each endpoint carries its own OAuth2 (client_credentials) block.
 */
...
/**
 * Extract every usable Event Mesh binding from a parsed default-env.json object.
 * Malformed bindings (missing management/amqp/httprest endpoint, OAuth block, or
 * namespace) are skipped rather than throwing, so a single bad entry never hides
 * the rest.
 */
export function extractEventMeshBindings(defaultEnv: unknown): EventMeshBinding[] {
  if (!isRecord(defaultEnv)) {
    return [];
  }
